...
- **`query` gained absolute-window + timezone support (`--end` / `--tz`).** `--since` only ever looked back from *now*, so investigating an alarm that fired earlier meant hand-computing Unix timestamps and dropping to raw `curl /api/v1/query_range` — and this fleet's alarms report **Beijing time (UTC+8)** while VM stores UTC, so every drill-in needed manual `-8h` math (and got it wrong at least once). `query --end` now pins the window's end at an absolute instant (accepts unix seconds, RFC3339, or a zone-less `"2006-01-02 15:04:05"`); combined with `--since` it selects exactly `[end-since, end]`. `--tz` sets the timezone that zone-less `--end` values are read in (IANA name like `Asia/Shanghai`, or offset `+08:00` / `+0800` / `+8` / `-05:00`) and adds a `time_local` column to the raw output so rows line up with the alarm's local clock. Default is UTC with no `time_local` column (unchanged contract). Passing `--end` alone (no `--since`) uses the same 5m default window as `--raw`. Bad `--end` / `--tz` return `FLAG_INVALID` (exit 2) with a hint.
- **`query` gained range support (`--since` / `--step`).** The escape-hatch `query` command was instant-only, so `query --since ...` hard-errored `unknown flag: --since` and the global `--raw` flag silently no-op'd back to an instant query with an empty `mode: ""`. Agents drilling into a scenario's `queries[].expr` to find *when* a metric peaked had no way to fetch the underlying time series. `query` is now instant by default but switches to a `/api/v1/query_range` call when `--since` or `--raw` is passed, emitting the same flattened raw shape as scenarios (`mode: "raw"`, `columns: [instance, timestamp, time, value]`, plus a `range` block). `--step` defaults to `auto`; `--raw` without `--since` uses a 5m window. Invalid `--since` / `--step` now return `FLAG_INVALID` (exit 2) with a hint instead of `INTERNAL`. Instant `query` now correctly reports `mode: "instant"` instead of an empty string.
...
### Added — `read-write-split` scenario
- **New hybrid scenario `read-write-split`** answers "is this latency read-driven or write-driven?" in one call. Per-RegionServer it emits `read_qps` / `write_qps` (from the `readrequestratepersecond` / `writerequestratepersecond` gauges), their `write_read_ratio`, and `wal_sync_p99_ms` (`synctime_99th_percentile{sub="WAL"}`). A p999 alarm that lines up with a `wal_sync_p99_ms` spike and `write_read_ratio` >>1 is write-path/WAL-fsync latency; a low ratio points at the read path (blockcache/compaction). Motivated by real incident triage that otherwise required hand-written PromQL to find the right metric names and `sub` labels. Scenario count 13 → 14.
- **Documented the silent-no-data metric traps** in CLAUDE.md and SKILL.md: `sub="WAL"` (WAL fsync) vs `sub="IPC"` (RPC call time) on similarly-named latency metrics; read/write rate living on `*ratepersecond` gauges rather than the `*requestcount` counters (`rpcmutaterequestcount` reads 0 on this fleet); and per-table × per-RS distribution being unavailable from VM (use balancer `*costfunction` series instead).
