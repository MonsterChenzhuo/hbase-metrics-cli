#!/usr/bin/env bash
# hbase-metrics-cli installer / upgrader.
...
#   SKILL_DIR=~/.claude/skills/hbase-metrics  install directory for the bundled Claude Code skill
#   NO_SUDO=1                                 never use sudo; fail if PREFIX is not writable
#   NO_SKILL=1                                skip installing the bundled skill
#   REPO=MonsterChenzhuo/hbase-metrics-cli    override repo slug

set -euo pipefail

REPO="${REPO:-MonsterChenzhuo/hbase-metrics-cli}"
PREFIX="${PREFIX:-/usr/local/bin}"
SKILL_DIR="${SKILL_DIR:-$HOME/.claude/skills/hbase-metrics}"
VERSION="${VERSION:-}"
...
archive="hbase-metrics-cli_${ver_no_v}_${os}_${arch}.tar.gz"
...
[ -x "${tmpdir}/hbase-metrics-cli" ] || die "binary not found in archive"
...
info "installing binary to ${PREFIX}/hbase-metrics-cli"
$sudo_cmd install -d "$PREFIX"
$sudo_cmd install -m 0755 "${tmpdir}/hbase-metrics-cli" "${PREFIX}/hbase-metrics-cli"

skill_src="${tmpdir}/.claude/skills/hbase-metrics"
...
installed_version=$("${PREFIX}/hbase-metrics-cli" version 2>/dev/null || echo "$VERSION")
info "done: ${installed_version}"
info "run: hbase-metrics-cli --help"
